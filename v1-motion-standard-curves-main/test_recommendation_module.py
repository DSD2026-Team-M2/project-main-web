...
def synthetic_squat_curve(repetitions=5):
    values = []
    for _ in range(repetitions):
        values.extend([30.0, 50.0, 95.0, 50.0, 30.0])
    return [
        {"time_seconds": index * 0.5, "angle": angle, "percent": None}
        for index, angle in enumerate(values)
    ]
...
class RecommendationModuleTests(unittest.TestCase):
    ...
    def test_overlay_timeline_places_multiple_squat_segments(self):
        standard_rows = build_standard_rows()
        patient_curve = synthetic_squat_curve(repetitions=5)
        session_start_ms = 1_700_000_000_000

        _segments, segmentation = rec.segment_patient_curve(patient_curve, "squat", "auto", smooth_window=1)
        self.assertTrue(segmentation["used"])
        self.assertGreaterEqual(segmentation["segmentsUsed"], 3)
        ...
        for index, segment in enumerate(segmentation["segments"]):
            if index > 0:
                points.append(rec.overlay_timeline_break())
            ...
            points.extend(rec.map_standard_rows_to_time_range(standard_rows, start_ms, end_ms))

        breaks = sum(1 for point in points if point["timeMs"] is None)
        self.assertEqual(breaks, segmentation["segmentsUsed"] - 1)
        plotted = [point for point in points if point["timeMs"] is not None]
        self.assertGreater(len(plotted), segmentation["segmentsUsed"] * 2)

        peak_count = 0
        for index in range(1, len(plotted) - 1):
            prev_angle = plotted[index - 1]["angle"]
            angle = plotted[index]["angle"]
            next_angle = plotted[index + 1]["angle"]
            if angle > prev_angle and angle >= next_angle:
                peak_count += 1
        self.assertGreaterEqual(peak_count, segmentation["segmentsUsed"])
