...
def load_patient_session_from_api(base_url, session_id, angle_id):
    ...
    return relative_curve, start_t


def load_patient_curve_from_api(base_url, session_id, angle_id):
    relative_curve, _start_t = load_patient_session_from_api(base_url, session_id, angle_id)
...
def overlay_timeline_break():
    return {"timeMs": None, "angle": None, "bandLow": None, "bandHigh": None}


def map_standard_rows_to_time_range(standard_rows, start_time_ms, end_time_ms):
    ...
    for row in standard_rows:
        percent = row["percent"]
        time_ms = start_time_ms + duration_ms * percent / 100.0
        point = {
            "timeMs": round(time_ms),
            "angle": row["standard_angle"],
        }
        if row.get("lower") is not None:
            point["bandLow"] = row["lower"]
        if row.get("upper") is not None:
            point["bandHigh"] = row["upper"]
        points.append(point)
    return points


def build_session_standard_overlay(
    action,
    session_id,
    standard_csv_path,
    ...
    standard_rows = load_standard_curve(standard_csv_path)
    patient_curve, session_start_ms = load_patient_session_from_api(base_url, session_id, angle_id)
    _segments, segmentation = segment_patient_curve(patient_curve, action, "auto", smooth_window)

    points = []
    segment_bounds = segmentation.get("segments") or []
    use_segmented_overlay = (
        action in ("walking", "squat")
        and segmentation.get("used")
        and len(segment_bounds) > 0
    )

    if use_segmented_overlay:
        overlay_mode = "segmented"
        for index, segment in enumerate(segment_bounds):
            if index > 0:
                points.append(overlay_timeline_break())
            start_ms = session_start_ms + float(segment["startTimeSeconds"]) * 1000.0
            end_ms = session_start_ms + float(segment["endTimeSeconds"]) * 1000.0
            points.extend(map_standard_rows_to_time_range(standard_rows, start_ms, end_ms))
    else:
        session_times = [row["time_seconds"] for row in patient_curve]
        start_ms = session_start_ms
        end_ms = session_start_ms + max(session_times) * 1000.0
        points = map_standard_rows_to_time_range(standard_rows, start_ms, end_ms)
        if action == "upstairs":
            overlay_mode = "full_session"
        elif action in ("walking", "squat"):
            overlay_mode = "full_session_fallback"
        else:
            overlay_mode = "full_session"
    ...
    return {
        ...
        "segmentsUsed": len(segment_bounds) if use_segmented_overlay else 1,
        "segmentation": segmentation,
        "sessionStartMs": session_start_ms,
        "standardSource": standard_csv_path,
        "points": points,
    }
